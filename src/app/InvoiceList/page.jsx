...
const InvoiceList = () => {
  ...
  return (
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {datas.map((invoice, index) => (
            <div key={index} className="bg-white rounded-none
             shadow-md overflow-hidden transition duration-300 ease-in-out transform hover:scale-105">
              <Link href={`/getOneInvoice/${invoice._id}`}>
                <div className="p-6 bg-gradient-to-r from-blue-500 to-purple-600 [h-140px] text-white">
                  <h2 className="font-bold text-xl mb-2">Client: {invoice.client}</h2>
                  <p className="text-sm opacity-75">Project: {invoice.project}</p>
                </div>
              </Link>
              <div className="p-6">
                <div className="mb-4">
    ...
    <li key={index}>{service.trim()}</li>
                  ...
                  <p className="text-sm text-gray-600"><span className="font-semibold">PIN:</span> {invoice.pin}</p>
                </div>
